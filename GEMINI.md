...
1. **Detect.** Before finishing any task, run the layout-agnostic drift gate (it survives the `crates/*` restructure — it discovers crates by their `Cargo.toml`, checks the core crate's own `[dependencies]`, not a lockfile count):
   ```bash
   bash .gemini/skills/merge-verification/scripts/drift-check.sh .   # exit 0 = clean
   ```
   Be especially alert to agent tooling that *auto-generates a package in another language or format* (a stray `.omc`, an `ecc`-style auto-pushed package, a Node/Python helper, a new dependency **on the core crate**) and commits it as a convenience. Treat any such artifact as drift to be caught, not adopted. (Legitimately non-Rust asset trees — `intent-driven-template/`, `.gemini/`, `.github/`, `docs/`, `openspec/` — are whitelisted by the gate.)
