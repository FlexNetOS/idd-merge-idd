...
- [ ] A1 · Add a **`cargo audit` (deny-warnings) CI gate** + commit an advisory baseline. Gate: CI job fails on any new advisory. Rollback: drop the job. (No code downgrade.)
- [ ] A2 · Remediate **RUSTSEC-2026-0009** — `time` 0.3.41 → **≥0.3.47** via `cargo update -p time` (forward only). Gate: `cargo audit` no longer reports `time`; `test --workspace --locked` still 429+. Rollback: revert Cargo.lock hunk.
