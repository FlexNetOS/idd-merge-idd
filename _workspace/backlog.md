...
- [ ] A5 · **Pin flake.nix Rust toolchain** (`crates/tui/flake.nix`) to **≥1.88** (the real MSRV floor measured in A4 — `time@0.3.47` + `let`-chains; NOT the originally-guessed 1.85) so the nix dev shell can't drift below the floor. Gate: `nix develop` builds. Rollback: revert flake.
