...
# Self-restarts the loop with a FRESH context each iteration (each `gemini -p`
...
#
#   bash .gemini/skills/idd-merge-loop/scripts/ralph-idd.sh                 # SAFE: plan/dry-run, commit non-destructive progress
...
command -v gemini >/dev/null || { log "FATAL: gemini not on PATH"; exit 1; }
...
while :; do
  ...
  gemini -p "$PROMPT" --model "$MODEL" --include-directories "$WORKTREE" "${APPLY_ARGS[@]}" \
